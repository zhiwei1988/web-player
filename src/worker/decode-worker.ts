...
import type { AudioDecoderConfig, WorkerRequest, WorkerResponse } from '../js/decoder/types.js';
...
self.onmessage = async (event: MessageEvent<WorkerRequest>) => {
    ...
    try {
        switch (request.type) {
            ...
            case 'initAudio':
                await handleInitAudio(request.config);
                break;
...
async function handleInitAudio(config: AudioDecoderConfig): Promise<void> {
    if (!decoder) {
        throw new Error('Decoder not initialized');
    }

    await decoder.initAudio(config.audioCodecType, config.sampleRate, config.channels);
...
async function handleDecode(data: Uint8Array, _pts?: number): Promise<void> {
    ...
    if (parsed.status === FrameParseStatus.SKIP || parsed.status === FrameParseStatus.ERROR) {
        return;
    }
    ...
    if (parsed.msgType === 0x01) {
        ...
        if (frame) {
            const transferableBuffers = [frame.yData.buffer, frame.uData.buffer, frame.vData.buffer];
    ...
    } else if (parsed.msgType === 0x02) {
        // Audio frame
        const audioFrame = await decoder.decodeAudio(parsed.payload, parsed.timestamp);
        ...
        if (audioFrame) {
            ...
            self.postMessage(response, [audioFrame.data.buffer]);
        }
    }
}
